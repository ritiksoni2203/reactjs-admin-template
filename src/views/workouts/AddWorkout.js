      ...
      <Card className="p-3 mt-2">
        <Formik
          initialValues={{
            ...
            caloriesBurned: state?.state?.caloriesBurned ?? '',
            date: state?.state?.date ?? ''
          }}
          validationSchema={Yup.object().shape({
            workoutType: Yup.string().required("Workout type is required"),
            duration: Yup.number().required("Duration is required"),
            caloriesBurned: Yup.string().required("Calories burned is required"),
          ...
          onSubmit={(values) => {
            ...
            id ? dispatch(updateWorkout({ data: values, id })) : dispatch(addWorkout({ data: values }));
          }}
        >
          {({
            values,
            ...
            <Form onSubmit={handleSubmit} onReset={handleReset}>
              ...
              <div className='mb-1 w-50'>
                ...
                <InputGroup>
                  ...
                  <Field type="number" className="form-control" id='duration' placeholder='Duration' name="duration"
                    onChange={(e) => setFieldValue("duration", e.target.value)}
                  />
                </InputGroup>
                {errors.duration && touched.duration ? (
                  <p className={"text-danger mb-0 error-form"}>{errors.duration}</p>
                ) : null}
              </div>
              <div className='mb-1 w-50'>
                ...
                <InputGroup>
                  ...
                  <Field type="number" className="form-control" id='date' placeholder='Calories Burned' name="caloriesBurned"
                    onChange={(e) => setFieldValue("caloriesBurned", e.target.value)}
                  />
                </InputGroup>
                {errors.caloriesBurned && touched.caloriesBurned ? (
                  <p className={"text-danger mb-0 error-form"}>{errors.caloriesBurned}</p>
                ) : null}
              </div>
              <div className='mb-1 w-50'>
                ...
                <InputGroup>
                  ...
                  <Field type="date" className="form-control" id='date' placeholder='Date' name="date"
                    onChange={(e) => setFieldValue("date", e.target.value)}
                  />
                </InputGroup>
                {errors.date && touched.date ? (
                  <p className={"text-danger mb-0 error-form"}>{errors.date}</p>
                ) : null}
              </div>
